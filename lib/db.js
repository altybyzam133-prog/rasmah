...
if (!columns('users').includes('email_verified')) {
  db.exec('ALTER TABLE users ADD COLUMN email_verified INTEGER NOT NULL DEFAULT 0');
  db.exec('ALTER TABLE users ADD COLUMN verify_code_hash TEXT');
  ...
  // shipped — the ALTER above defaults new AND existing rows to 0, and
  // without this every current user (including real ones already using the
  // site) would get locked out of their own account on their next visit
  // until they "verify" an address they've already been using for weeks.
  // Only accounts created from this point on start at 0 and must verify.
  db.exec('UPDATE users SET email_verified = 1');
}

// ---- users --------------------------------------------------------------
// Deliberately lenient shape check only (just "text@text", no required
// TLD/dot) — this used to require a dot in the domain part (`x@y.z`), which
// rejected obviously-fake-looking but still email-shaped input like
// `test@test`. Whether the address is REAL is no longer this regex's job at
// all: registration now emails a 6-digit code (see setEmailVerifyCode below)
// and the account can't be used until that code is entered back, so a fake
// or mistyped address just never receives the code and the account stays
// permanently unverified — a much stronger check than any regex.
const EMAIL_RE = /^[^\s@]+@[^\s@]+$/;
...
function updateProfile(id, { name, email }) {
  ...
  // Switching to a different address must re-arm verification — otherwise
  // someone could verify a real inbox once at signup, then freely swap in
  // any fake address afterward with zero friction.
  const emailChanged = !current || current.email_lower !== emailLower;
  try {
    db.prepare(
      emailChanged
        ? 'UPDATE users SET name = ?, email = ?, email_lower = ?, email_verified = 0 WHERE id = ?'
        : 'UPDATE users SET name = ?, email = ?, email_lower = ? WHERE id = ?'
    ).run(name, email, emailLower, id);
  } catch (err) {
...
// ---- email verification ----------------------------------------------------
// Same shape as password reset above (6-digit code, bcrypt-hashed at rest),
// but a longer TTL since it's gating account activation rather than a
// security-sensitive action — a user may not open their inbox right away,
// and "resend" (routes/auth.js) covers the case where it does expire.
const VERIFY_CODE_TTL_MS = 24 * 60 * 60 * 1000;

function setEmailVerifyCode(id, code) {
  const hash = bcrypt.hashSync(String(code), 8);
  db.prepare('UPDATE users SET verify_code_hash = ?, verify_code_expires = ? WHERE id = ?')
    .run(hash, Date.now() + VERIFY_CODE_TTL_MS, id);
}
function verifyEmailCode(id, code) {
  const u = getUserById(id);
  if (!u || !u.verify_code_hash || !u.verify_code_expires) return false;
  if (Date.now() > u.verify_code_expires) return false;
  return bcrypt.compareSync(String(code || ''), u.verify_code_hash);
}
function markEmailVerified(id) {
  db.prepare('UPDATE users SET email_verified = 1, verify_code_hash = NULL, verify_code_expires = NULL WHERE id = ?').run(id);
}
