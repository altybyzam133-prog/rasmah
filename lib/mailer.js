...
async function sendMail({ to, subject, text, html }) {
  if (!transporter || config.isTest) {
    // In test mode (smoke-test.js sets NODE_ENV=test), always log instead of
    // really sending — regardless of whatever real credentials happen to be
    // sitting in the developer's own .env — so the suite stays deterministic,
    // offline, and never leaks a real test email to a real inbox.
